<!-- THORONDOR // component=meneldor-readme // stack=802.11 // status=accepted -->
# Meneldor — radioctl + radio-inject (heterogeneous radio control plane & L2.5 injector)

> **Placement:** this file goes at `Meneldor/README.md` — a folder at the **repo root**,
> sibling to `Gwaihir/` and `Westron/`. This is the OpenWrt-hosted **802.11** tooling; its
> code, tests, and reports live in the same folder. Paths in this doc are written relative
> to the repo root (e.g. `Meneldor/radioctl.sh`).
> (Change note: repo flattened — no `thorondor/` wrapper; paths updated from
> `thorondor/meneldor/…` to `Meneldor/…`.)
...
```bash
# Linux
bash Meneldor/radioctl.sh --config Meneldor/radios.conf          # REPL
bash Meneldor/radioctl.sh --config Meneldor/radios.conf doctor   # deps
python3 Meneldor/radio-inject selftest                                     # acceptance check
```
```powershell
# Windows
.\thorondor\meneldor\radioctl.ps1 -Config thorondor\meneldor\radios.conf
.\thorondor\meneldor\radioctl.ps1 -Config thorondor\meneldor\radios.conf doctor
```
...
Reproduce:
```bash
python3 Meneldor/montecarlo_radio_inject.py 20000 0xC0FFEE   # RESULT: PASS
bash    Meneldor/montecarlo_radioctl.sh 150                   # RESULT: PASS
```
```powershell
pwsh -File Meneldor/montecarlo_radioctl.ps1 -Runs 2000        # run on Windows
```
...
`.github/workflows/regression.yml` (at the repo root — GitHub only runs workflows there)
fires on pushes/PRs that touch `Meneldor/**`, and on manual dispatch:

- **linux** (ubuntu-latest): `radio-inject selftest`, `montecarlo_radio_inject.py`,
  `montecarlo_radioctl.sh`, plus informational shellcheck — all under `Meneldor/`.
- **windows** (windows-latest): `radioctl.ps1 doctor` smoke + `montecarlo_radioctl.ps1`.
- **gate**: green only if both matrices pass.

This turns the Monte Carlo campaign into a merge gate so no change can regress the verified
logic. (The path filter keeps it from firing on unrelated changes; remove it to run on every push.)
